...
export default function Hero() {
  ...
  // Animate text lines in when showContent becomes true
  useEffect(() => {
  ...
  // Show content after video plays once (~6s) + 2s extra buffer
  useEffect(() => {
    const t = setTimeout(() => setShowContent(true), 6000);
    return () => clearTimeout(t);
  }, []);

  // Content fades and rises slightly as we scroll away from hero
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Background video — full bleed, no zoom */}
          ...
          {showContent && (
            <div className="max-w-4xl mx-auto flex flex-col items-center text-center">
              ...
              >
                ...
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-[#F59E0B]">
                  Your Local Pressure Washing Professionals
                </span>
              </motion.div>

              <h1
                className="text-4xl md:text-7xl lg:text-8xl font-black text-white tracking-tighter leading-[1.1] mb-8 uppercase"
              ...
              >
                {HERO_LINES.map((line, i) => (
                  <span
                    ...
                    className={`block ${i === 2 ? "bg-clip-text text-transparent bg-gradient-to-r from-[#F59E0B] to-[#FCD34D]" : ""}`}
                    style={{
              ...
              <motion.p
                ...
                className="text-base md:text-xl text-white/60 max-w-2xl mb-12 font-medium"
              >
              ...
              >
                <Link
                  href="#contact"
                  className="group relative px-8 py-4 bg-[#F59E0B] text-[#0A1628] rounded-full font-black uppercase tracking-widest text-xs overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-lg shadow-[#F59E0B]/25"
                >
                ...
                <a
                  ...
                  className="px-8 py-4 bg-white/5 hover:bg-white/10 text-white rounded-full font-bold border border-white/10 backdrop-blur-md transition-all group"
                >
